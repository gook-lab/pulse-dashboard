...
다크 트레이딩 터미널 스타일의 시황 통합 대시보드예요.
지수·히트맵·뉴스(감성)·한국 실시간 체결/호가·순위·포트폴리오(KIS 모의계좌)·
AI 종합의견·기술적 목표가에 더해, 국내 부동산 모듈(실거래·단지투어 3D 배치도)까지 한 화면에 담았어요.
...
> 백엔드 미연결 상태. 실데이터가 없는 자리는 목이 아니라 `-`로 표시돼요 — 의도된 동작이에요.
...
> API 키는 `server/.env`에 둬요. **절대 프론트로 내리거나 커밋하면 안 돼요.**
> `pnpm validate`는 훅과 달리 경고가 아니라 **막아요** — 커밋 전에 돌려야 해요.
...
FE 아키텍처의 **단일 소스는 [`docs/ARCHITECTURE-FE.md`](docs/ARCHITECTURE-FE.md)**예요 (RADIO 형식:
Requirements → Architecture → Data model → Interface → Optimization/Observability).
품질 기준을 "빠르다" 같은 말이 아니라 **수치 SLI**로 적어 두고 그걸 지켜요.

### 불변식 5개 (절대 규칙)

1. **브라우저는 KIS에 직결하지 않아요.** 실시간은 서버 WebSocket 게이트웨이 1개 →
   SSE 팬아웃(`/api/stream`)만 써요. 키·재접속·키 한도는 서버 책임이에요.
2. **실데이터가 없으면 목이 아니라 `-`예요.** `unavailable` / `targetReal` 플래그로 표현해요.
   httpApi 실패 경로에서 `mockApi`를 반환하면 안 돼요.
3. **등락색은 주입돼요.** `signColor(pct, mode)`만 쓰고 hex를 하드코딩하면 안 돼요.
   `colorMode`(글로벌 초록↑빨강↓ / 한국 빨강↑파랑↓)가 전역이라, 컴포넌트가 색을 정하면 안 돼요.
4. **파서 계약은 골든 테스트로 잠가요.** SSE 체결/호가 필드 인덱스가 바뀌면 픽스처부터 고쳐요.
5. **캐시는 빈 성공을 저장하면 안 돼요.** 스로틀 순간의 `{}`가 TTL 동안 박제되면 골치 아파요.
...
폴링은 전부 `document.hidden` 게이트를 걸어요.

---

## 외부 API에서 배운 것

이 프로젝트의 실질적 난이도는 UI가 아니라 **남의 API를 견디는 일**이었어요.

- **CNN · data.go.kr는 User-Agent가 없으면 차단돼요.**
- **data.go.kr는 50콜 동시 호출 시 스로틀** — 동시성 5 + 재시도로 눌렀어요.
- **KIS 토큰은 1분에 1회** — in-flight 중복 제거 + 캐시로 처리했어요.
- **모든 KIS 호출은 `kisFetch` 게이트를 통과해야 해요.** 모의계좌는 초당 상한이 낮아서
  라우트가 각자 던지면 서로를 밀어낸다고 해요(실측: 서로 다른 종목 연속 조회에서 6/10 실패).
  전역 직렬 큐 + **적응형 간격**(5xx/429면 ×1.6, 성공하면 −15ms, 200~1600ms)으로 0/12까지 내렸어요.
  고정 간격은 TR마다 상한이 달라서 항상 틀렸어요.
  > ⚠️ 게이트에서 자동 재시도하면 안 돼요. 주문(`kisPost`)까지 재시도되면 **중복 주문**이 된다고요.
  > 재시도는 호출자 책임이에요.
- **목 데이터는 실제와 3배 이상 벌어져요** (삼성전자 시총 468조 vs 1,552조 · PER 12.8 vs 40.5).
  그래서 불변식 2번이 있어요.
...
**three.js 없이** OSM 건물 외곽선을 SVG로 압출해요. 회전은 좌표를 돌리는 것이라
렌더러가 필요 없어요.

- 방위 yaw 0~360° 연속, 고도 pitch 8~85°(기본 30°). 지면 깊이는 `sinφ`, 높이는 `cosφ`로 눌려요.
- **깊이 정렬 축은 yaw와 함께 돌아야 해요** — 월드 `x+y`로 정렬하면 180°에서 순서가
  뒤집혀서 뒷건물이 앞을 덮더라고요.
- **OSM 링 방향은 보장되지 않아요** (실측 CCW 451 / CW 209). 벽 법선을 링 방향으로만
  구하면 32%가 명암 반전되거든요 — `ringIsCCW`로 먼저 쟀어요.
- **LOD** — 축소하면 덜 그려요. 최소 건물 면적 0/150/400m².
- **주변 스트리밍** — 200m 격자 칸 단위. 칸 캐시는 원점과 무관하게 구워서 **단지끼리 공유**돼요.
  단지를 바꾸면 진행 중 요청을 끊고 응답의 `seq`를 대조해요 (늦게 온 응답이 새 좌표계에
  섞이면 건물이 엉뚱한 곳에 서거든요).
- **지하철역은 대부분 터널**이라서 지오메트리 규칙 `isHidden`을 표시에 그대로 적용하면
  역세권이 통째로 사라져요. 도로명은 `residential`까지 포함해야 주택가에서 0개가 아니에요.
...
개인 학습용 토이 프로젝트예요. **투자 조언이 아니에요.**
KIS 연동은 검증 전까지 **모의(mock) 계좌만** 사용해야 해요.
`server/data/`(개인 금융 데이터)와 `server/.env`(키)는 `.gitignore`에 있어요 — 커밋하면 안 돼요.
...
사용 권한을 드린 건 아니에요. 다른 프로젝트에 가져다 쓰거나 재배포·상업적 이용을
